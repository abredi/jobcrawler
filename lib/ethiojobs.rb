...
class EthioJobs < Scrapper
  ...
  def initialize
    @base_url = 'http://www.ethiojobsXXXXX.net/search-results-jobs'
    super(self)
    snatch_data
  end
      ...
      @current_page.push(
        {
          ...
          link: link.css('h2 a').text.strip,
          company: link.css('.company-name').text.strip,
          company_link: link.css('.company-name').text.strip,
          work_place: link.css('.work-palce').text.strip,
          level: link.css('.pull-left > span:nth-child(4)').text.strip,
